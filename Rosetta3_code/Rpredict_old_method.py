...
# python Rpredict.py -h                                              # provides help
...
# tested on python 2.7      --> updated for Python 3.7.3
import numpy as N
import argparse
import time as T
...
# you want to change the username
...
parser.add_argument('--predict', action='store_true',
                    help='get data and predict')
parser.add_argument('--input', help='input file path')
parser.add_argument('--check', action='store_true',
                    help='check prediction (for development only)')
parser.add_argument('--raw', action='store_true', help='get raw data')
# using this option will ignore the mysql options.
parser.add_argument('--sqlite', default='',
                    help='sqlite path (ignores MySQL options)')
...
def get_rosetta_cal_data(db, input_var):
    ...
    data_query_clause = "SELECT " + \
        ",".join(ptf_model.input_var) + " FROM `Data`"
...
if args.predict:
    ...
    with DB(host=args.host, user=args.user, db_name=args.db, sqlite_path=args.sqlite) as db:
        ...
        # theta_r [cm3/cm3]
        # theta_s [cm3/cm3]
        # log10(alpha) in log10(1/cm)
        # log10(n)
        # log10(Ks) in log10(cm/day)
        ...
        data = N.loadtxt(open("lfu_bkdat.csv", "rb"), delimiter=",", skiprows=1,dtype=N.float).transpose()
        ...
        if not args.raw:
            ...
            print(res_dict['sum_res_cov'][:, :, :])
            print(data)
            nm = res_dict['var_names']
            a = res_dict['sum_res_mean']
            b = res_dict['sum_res_std']
            c = N.concatenate((a, b), axis=0)
            outfile = "all_old_method.npy" #outfile = "sum-res-mean.npy"
            N.save(outfile, c)
            outfile2 = "names_old_method.npy"
            N.save(outfile2, nm)
