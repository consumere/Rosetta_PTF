...
class ANN_MODEL(object):
    ...
    def __init__(self, model_id, db):
        # load as bytes as loading as string would mess up the network weights
        ...
        # decode the transfer function from bytes to utf-8 is important
        self.ann_sequence=[ANN.from_stream(io.BytesIO(ann_bin), nlayer=nlayer, transfers=[nhid1_transfer.decode("utf-8"), nhid2_transfer.decode("utf-8"), nout_transfer.decode("utf-8")]) for (hash_id, seq, model_id, nin, nlayer, nhid1, nhid1_transfer, nhid2, nhid2_transfer, nout, nout_transfer, ann_bin) in tmp_list]
